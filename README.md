...
## 概要
- ASP.NET Coreを用いたWebアプリの開発が可能で、MVCアーキテクチャ/依存性注入(DI)/SOLID原則/各種デザインパターン(Composite/Decorator/Command/Observer/State/Template Method/Strategyなど）/自動テストなどを活用し、保守性を考慮した設計・実装をします。各種脅威（XSS/CSRF/DoS攻撃/OSインジェクション/セッションハイジャック/SQLインジェクション/通信の盗聴/パスワード流出）への対策（HTMLエスケープ/CSRFトークン/通信暗号化/アクセス数制限/OSコマンド不使用/2要素認証/セッションID更新/ORM/IP制限）を実施し、セキュリティを強化します。また、CI/CDに脆弱性診断スクリプトを組み込み、脆弱性の早期検知を実現します。
